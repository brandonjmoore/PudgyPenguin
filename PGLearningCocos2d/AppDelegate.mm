...
- (void) applicationDidFinishLaunching:(UIApplication*)application
{
    ...
    //Load high scores
    NSString *settingsPath = [documentsDirectory stringByAppendingPathComponent:@"MySettings.plist"];
    if ([[NSFileManager defaultManager] fileExistsAtPath:settingsPath]) {
        maxLevelDictionary = [[NSMutableDictionary alloc]initWithContentsOfFile:settingsPath];
    } else {
        maxLevelDictionary = [[NSMutableDictionary alloc] initWithObjects:[[NSArray alloc]initWithObjects:[NSNumber numberWithInt:1], nil] forKeys:[[NSArray alloc]initWithObjects:@"MaxLevelUnlocked", nil]]; 
    }
...
- (void)applicationWillResignActive:(UIApplication *)application {
    ...
    if (current.tag > 0) {
        ...
        if (!layer.isPaused && !layer.gameOver) {
            [layer doPause];
        }
...
#pragma mark -
#pragma mark Max Unlocked Level
...
-(void)saveMaxLevelUnlocked:(NSNumber*)levelNumber {
    ...
    NSString *key = @"MaxLevelUnlocked";
    
    NSNumber *value = [maxLevelDictionary objectForKey:key];
    
    
    
    if (value == nil || [levelNumber compare:value] == NSOrderedDescending) {
        [maxLevelDictionary setObject:levelNumber forKey:key];
        [maxLevelDictionary writeToFile:path atomically:YES];
    }
}

-(NSInteger)getMaxLevelUnlocked {
    NSNumber *maxLevelUnlocked = [maxLevelDictionary objectForKey:@"MaxLevelUnlocked"];
    return [maxLevelUnlocked integerValue];
}
