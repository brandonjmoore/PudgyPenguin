...
-(void)addFish {
    ...
    if (penguin2 != nil) {
        if (!gameOver) {
            Box2DSprite * fish = [self createFish2AtLocation:ccp(screenSize.width * .1,screenSize.height * -.1)];
            numFishCreated++;
...
-(void)doHighScoreStuff {
    ...
    if (remainingTime > [app getHighScoreForLevel:kLevel24]) {
...
-(id)initWithLevel24UILayer:(UILayer *)level24UILayer {
    if ((self = [super init])) {
        ...
        [FlurryAnalytics logEvent:@"Level 24 Started"];
        ...
        [self createPenguin2AtLocation:ccp(winSize.width * .1, winSize.height * .8)];
