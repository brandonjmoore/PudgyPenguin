...
@implementation Level28ActionLayer
...
-(void) doResetLevel {
    ...
    [[GameManager sharedGameManager] runSceneWithID:kGameLevel28];//Level Specific: Change for new level
}
...
-(void) doNextLevel {
    ...
    [defaults setBool:YES forKey:@"level29unlocked"];
    [defaults synchronize];
    if (appDel != nil) {
        [appDel saveMaxLevelUnlocked:[NSNumber numberWithInt:29]];
    }
...
-(void)doHighScoreStuff {
    ...
    if (remainingTime > [app getHighScoreForLevel:kLevel28] && [app getHighScoreForLevel:kLevel28] > 0) {
    ...
    [app setHighScore:[NSNumber numberWithDouble:remainingTime] forLevel:kLevel28];
    
    NSInteger levelHighScore = [app getHighScoreForLevel:kLevel28];
    NSString *levelScoreString = [NSString stringWithFormat:@"Level 28 high score: %d", levelHighScore];
...
-(id)initWithLevel28UILayer:(UILayer *)level28UILayer {
    if ((self = [super init])) {
        ...
        [self createPenguin2AtLocation:ccp(winSize.width * .25, winSize.height * .1)];
        penguin2 = (Penguin2*)[sceneSpriteBatchNode getChildByTag:kPenguinSpriteTagValue];
        
        for (float i = .0425; i < 1; i=i+.081) {
        ...
        [self createBoxAtLocation:ccp(winSize.width * .75,winSize.height * .05) ofType:kBouncyBox withRotation:CC_DEGREES_TO_RADIANS(0)];
        
        Box2DSprite *block1 = [self createBoxAtLocation:ccp(winSize.width * .1,winSize.height * .5) ofType:kBouncyBox withRotation:CC_DEGREES_TO_RADIANS(15)];
        [block1 runAction:[CCRepeatForever actionWithAction:[CCRotateBy actionWithDuration:2 angle:360]]];
        
        Box2DSprite *block3 = [self createBoxAtLocation:ccp(winSize.width * .275,winSize.height * .4) ofType:kBouncyBox withRotation:CC_DEGREES_TO_RADIANS(50)];
        [block3 runAction:[CCRepeatForever actionWithAction:[CCRotateBy actionWithDuration:2 angle:360]]];
        
        Box2DSprite *block4 = [self createBoxAtLocation:ccp(winSize.width * .35,winSize.height * .6) ofType:kBouncyBox withRotation:CC_DEGREES_TO_RADIANS(0)];
        [block4 runAction:[CCRepeatForever actionWithAction:[CCRotateBy actionWithDuration:2 angle:360]]];
