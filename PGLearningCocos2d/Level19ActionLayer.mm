...
-(void)addFish {
    ...
    if (penguin2 != nil) {
        if (!gameOver) {
            [self createFish2AtLocation:ccp(screenSize.width * -.05,screenSize.height * .63)];
            numFishCreated++;
...
-(void)addTrash {
    ...
    if (penguin2 != nil) {
        if (!gameOver) {
            [self createTrashAtLocation:ccp(screenSize.width * 0.25f, screenSize.height * 1.1f)];
...
-(void) doResetLevel {
    ...
    [[GameManager sharedGameManager] runSceneWithID:kGameLevel19];//Level Specific: Change for new level
}
...
-(void) doNextLevel {
    ...
    [defaults setBool:YES forKey:@"level20unlocked"];
    [defaults synchronize];
    if (appDel != nil) {
        [appDel saveMaxLevelUnlocked:[NSNumber numberWithInt:20]];
    }
...
-(void)doHighScoreStuff {
    ...
    if (remainingTime > [app getHighScoreForLevel:kLevel19] && [app getHighScoreForLevel:kLevel19] > 0) {
    ...
    [app setHighScore:[NSNumber numberWithDouble:remainingTime] forLevel:kLevel19];
    
    NSInteger levelHighScore = [app getHighScoreForLevel:kLevel19];
    NSString *levelScoreString = [NSString stringWithFormat:@"Level 19 high score: %d", levelHighScore];
...
-(id)initWithLevel19UILayer:(UILayer *)level19UILayer {
    if ((self = [super init])) {
        ...
        [self createPenguin2AtLocation:ccp(winSize.width *.95, winSize.height * 0.8f)];
        penguin2 = (Penguin2*)[sceneSpriteBatchNode getChildByTag:kPenguinSpriteTagValue];
        
        [penguin2 setRotation:270];
        
        [self createPlatformAtLocation:ccp(winSize.width * -.05,winSize.height * .6) ofType:kMediumPlatform withRotation:CC_DEGREES_TO_RADIANS(-280)];

        [self createPlatformAtLocation:ccp(winSize.width * 0.85f, winSize.height *0.9f) ofType:kLargePlatform withRotation:0.0];
        
        [self createBoxAtLocation:ccp(winSize.width * .65,winSize.height * .15) ofType:kBouncyBox withRotation:CC_DEGREES_TO_RADIANS(-45)];
//        [self createBoxAtLocation:ccp(winSize.width * .05,winSize.height * .35) ofType:kBouncyBox withRotation:CC_DEGREES_TO_RADIANS(0)];
        
        
        for (float i = .32; i < .55; i=i+.08125) {
            [self createBoxAtLocation:ccp(winSize.width * .05,winSize.height * i) ofType:kBouncyBox withRotation:CC_DEGREES_TO_RADIANS(0)];
        }
