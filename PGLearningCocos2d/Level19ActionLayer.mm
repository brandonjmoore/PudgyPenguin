...
-(void)addFish {
    ...
    if (penguin2 != nil) {
        if (!gameOver) {
            [self createFish2AtLocation:ccp(screenSize.width * 0.6f, screenSize.height * 1.05f)];
            numFishCreated++;
...
-(void)addTrash {
    ...
    if (penguin2 != nil) {
        if (!gameOver) {
            [self createTrashAtLocation:ccp(screenSize.width * 0.8, screenSize.height * 1.05)];
...
-(void)doHighScoreStuff {
    ...
    if (remainingTime > [app getHighScoreForLevel:kLevel19]) {
...
-(id)initWithLevel19UILayer:(UILayer *)level19UILayer {
    if ((self = [super init])) {
        ...
        [self createBoxAtLocation:ccp(winSize.width * .05,winSize.height * .3) ofType:kBouncyBox withRotation:CC_DEGREES_TO_RADIANS(15)];
...
//        CCSequence *seq = [CCSequence actions:mov1,mov2,mov3, nil];
        ...
        //[self schedule:@selector(addTrash) interval:16];
