...
-(void) doNextLevel {
    ...
    if (appDel != nil) {
        [appDel saveMaxLevelUnlocked:[NSNumber numberWithInt:3]];
    }
...
-(void)doHighScoreStuff {
    ...
    if (remainingTime > [app getHighScoreForLevel:kLevel2] && [app getHighScoreForLevel:kLevel2] > 0) {
    ...
    [app setHighScore:[NSNumber numberWithDouble:remainingTime] forLevel:kLevel2];
    
    NSInteger levelHighScore = [app getHighScoreForLevel:kLevel2];
    NSString *levelScoreString = [NSString stringWithFormat:@"Level 2 high score: %d", levelHighScore];
...
-(id)initWithLevel2UILayer:(UILayer *)level2UILayer {
    if ((self = [super init])) {
        CGSize winSize = [CCDirector sharedDirector].winSize;
        
        [FlurryAnalytics logEvent:@"Level 2 Started"];
